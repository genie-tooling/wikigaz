...
import sys
import os
from typing import Dict, Any, List, Tuple, Optional, cast, Union, Set # Added Union, Set

# Make utils discoverable
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

from utils.config import load_config, get_config, pydash_get, get_base_dir, ConfigurationError # Added ConfigurationError
from utils.logging_config import setup_logging
from utils.mongo_helpers import get_mongo_client, get_database, get_entity_collection
from pymongo.errors import CollectionInvalid, OperationFailure, ConnectionFailure, ConfigurationError as MongoConfigError # Renamed pymongo's CE
    ...
    try:
        ...
        if isinstance(schema_doc, dict) and '$jsonSchema' in schema_doc:
            ...
            if not isinstance(schema, dict):
                 raise ValueError("'$jsonSchema' key does not contain a dictionary object.")
            logger.info(f"Successfully loaded MongoDB schema (extracted $jsonSchema) from {schema_file_path}")
            return cast(Dict[str, Any], schema)
        elif isinstance(schema_doc, dict) and 'bsonType' in schema_doc:
             # Assume the file *is* the $jsonSchema content directly
             logger.warning(f"Schema file {schema_file_path} does not contain top-level '$jsonSchema' key. Assuming file content is the schema itself.")
             schema = schema_doc
             return cast(Dict[str, Any], schema)
        else:
             raise ValueError("Loaded JSON does not appear to be a valid schema (missing '$jsonSchema' or 'bsonType').")
        ...
        raise
         ...
         logger.error(f"Unexpected error loading schema file {schema_file_path}: {e}", exc_info=True)
         raise
...
def apply_schema_validation(db: Database, collection_name: str, schema: Dict[str, Any]):
    ...
    Creates the collection with validation if it doesn't exist, otherwise uses 'collMod'.

    Args:
        db: The pymongo Database object.
        collection_name: The name of the collection to modify/create.
    ...
    try:
        ...
        if collection_name not in existing_collections:
            ...
            logger.info(f"Collection '{collection_name}' does not exist. Creating with schema validation...")
            db.create_collection(collection_name, **validator_options)
            logger.info(f"Successfully created collection '{collection_name}' with JSON schema validation.")
        else:
            # Modify existing collection's validator
            logger.info(f"Collection '{collection_name}' exists. Attempting to update schema validation using collMod...")
            db.command({
                "collMod": collection_name,
                **validator_options # Use the same validator options dictionary
            })
            logger.info(f"Successfully updated JSON schema validation for existing collection '{collection_name}'.")

    except OperationFailure as e:
        # Catch specific errors during create_collection or collMod
        logger.error(f"MongoDB operation failed applying/updating schema for collection '{collection_name}': {e.details}", exc_info=True)
        raise # Reraise critical failures
    except Exception as e:
        logger.error(f"An unexpected error occurred during schema application for '{collection_name}': {e}", exc_info=True)
...
def create_indexes(collection: Collection, schema: Dict[str, Any]):
    ...
    if not isinstance(properties, dict):
         logger.warning("Schema 'properties' field is missing or not a dictionary. Cannot infer indexes from schema.")
         properties = {}
    ...
    if 'coordinates' in properties:
        ...
        if isinstance(coord_prop, dict) and \
           pydash_get(coord_prop, 'properties.type.enum') == ['Point'] and \
           pydash_get(coord_prop, 'properties.coordinates.bsonType') == 'array':
    ...
    if 'admin_hierarchy' in properties:
         ...
         if isinstance(admin_prop, dict) and \
            admin_prop.get('bsonType') == 'array' and \
            pydash_get(admin_prop, 'items.properties.qid.bsonType') == 'string':
    ...
    if 'wiki_links' in properties:
         ...
         if isinstance(wiki_prop, dict) and isinstance(wiki_prop.get('properties'), dict):
             if pydash_get(wiki_prop, 'properties.normalized_enwiki_title.bsonType') == 'string':
                 indexes_to_ensure.add(("wiki_links.normalized_enwiki_title", pymongo.ASCENDING))
             if pydash_get(wiki_prop, 'properties.inlink_count.bsonType') in ['int', 'long', 'number', 'double']: # Allow various numeric types
                 indexes_to_ensure.add(("wiki_links.inlink_count", pymongo.DESCENDING)) # High counts first
    ...
    if 'embedding_vector' in properties:
         if pydash_get(properties, 'embedding_vector.bsonType') == 'array':
             ...
             logger.warning("Found 'embedding_vector' array in schema. Consider specialized vector indexes (e.g., Atlas Search) for efficient similarity search instead of a basic index.")
    ...
    try:
        ...
        logger.debug(f"Existing indexes: {existing_index_keys}")
         ...
         logger.error(f"Failed to get index information for collection '{collection.name}': {e.details}. Skipping index creation.")
    ...
    for field, direction in sorted_indexes:
        ...
        if index_key_tuple in existing_index_keys:
            ...
            logger.debug(f"- Index on key '{index_key_tuple}' already exists (named '{existing_name}'). Skipping creation of '{index_name}'.")
            skipped_count += 1
            continue

        # Attempt to create the index
        try:
            logger.info(f"- Creating index '{index_name}' on key '{index_key_tuple}'...")
            # background=True allows other operations while index builds (good for large collections)
            collection.create_index(list(index_key_tuple), name=index_name, background=True)
            ...
            created_count += 1
             ...
             logger.warning(f"- Could not create index '{index_name}' on key '{index_key_tuple}': {e.details}. It might conflict with an existing index or options.")
             failed_count += 1
        except Exception as e:
             logger.error(f"- Unexpected error creating index '{index_name}' on key '{index_key_tuple}': {e}", exc_info=True)
             failed_count += 1

    logger.info(f"Index creation summary: {created_count} created/initiated, {skipped_count} skipped (already exist), {failed_count} failed.")
...
def main():
    ...
    parser.add_argument(
        "--config", default="config/config.yaml", help="Path to the global YAML configuration file."
    )
    parser.add_argument(
    ...
    try:
        ...
        schema_file_path_from_config = pydash_get(config, 'mongodb.schema_file') # Check config first
        schema_file_path_rel = args.schema_file_arg or schema_file_path_from_config # Override with arg if provided
        if not schema_file_path_rel:
             logger.critical("MongoDB schema file path not found in arguments or configuration ('mongodb.schema_file').")
        ...
        if not collection_name:
            logger.critical("Entity collection name not found in configuration (mongodb.collection_entities).")
            raise ConfigurationError("Entity collection name is not configured.")

    except (FileNotFoundError, ConfigurationError, MongoConfigError, KeyError, json.JSONDecodeError, ValueError) as e:
         # Catch setup errors before attempting DB operations
         logger.critical(f"Setup script failed during configuration/schema loading: {e}", exc_info=True)
         sys.exit(1)
    except Exception as e:
         # Catch unexpected errors during setup
         logger.critical(f"An unexpected error occurred during setup phase: {e}", exc_info=True)
         sys.exit(1)

    # --- Execution Phase ---
    try:
        logger.info(f"--- Setting up MongoDB Collection: {db.name}.{collection_name} ---")

        # Apply Schema Validation (handles creation or modification)
        ...
        logger.critical(f"MongoDB operation failed during setup: {e.details if hasattr(e, 'details') else e}", exc_info=True)
        sys.exit(1)
    except Exception as e:
        logger.critical(f"An unexpected error occurred during MongoDB setup execution: {e}", exc_info=True)
        sys.exit(1)
